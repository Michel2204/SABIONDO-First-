...
import { obtenerPreguntaDePool, obtenerDificultad, dificultadKeyPorNivel } from "@/lib/preguntasCarrera"; // CAMBIO: nuevos imports
import { obtenerPoolCarrera } from "@/lib/preguntasApi"; // CAMBIO: nuevo import
import { Pregunta } from "@/lib/types"; // CAMBIO: nuevo import
...
export default function PantallaCarrera({ onVolver }: PantallaCarreraProps) {
  ...
  // CAMBIO: el pool ahora se trae de Supabase, uno por tramo de dificultad
  const [pool, setPool] = useState<Pregunta[] | null>(null);
  const dificultadKey = dificultadKeyPorNivel(nivel);

  useEffect(() => {
    let cancelado = false;
    setPool(null); // muestra "cargando" al cruzar de tramo (nivel 34, nivel 67)
  ...
  // Congelada por nivel: no se recalcula (ni remezcla el orden de opciones) en cada
  // render, solo cuando realmente cambiamos de nivel. Esto evita que el orden de las
  // opciones cambie entre que el usuario ve la pregunta y hace clic.
  const pregunta = useMemo(
    () => (pool ? obtenerPreguntaDePool(nivel, pool) : null),
    [nivel, pool]
  ); // CAMBIO: ahora depende de `pool`, y puede ser null mientras carga

  // Tick cada 1s solo mientras se muestra la cuenta regresiva
  useEffect(() => {
    if (vidas > 0) return;
  ...
  if (!cargado || !pregunta) { // CAMBIO: agregado `|| !pregunta`
    return (
  ...
  // Arrow functions (no "function" hoisteada) para que TS mantenga acotado
  // "pregunta" a Pregunta (no null) dentro del cierre, ya que arriba ya
  // devolvimos si pregunta era null.
